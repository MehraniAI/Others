...
# ================== VIEW GUEST LIST WITH FILTERS ==================
    ...
    # Filter by City/Village
    cities = df["City"].unique().tolist()
    selected_city = st.selectbox("Filter by City/Village (leave empty for all)", ["All"] + cities)
...
# ================== UPDATE GUEST ==================
    ...
    if df.empty:
        ...
        if selected:
            ...
            new_amount = st.number_input("Amount (PKR)", value=df.at[guest_index, "Amount"], min_value=0, step=100)
            if st.button("Save Update"):
                df.at[guest_index, "Name"] = new_name.strip()
                df.at[guest_index, "City"] = new_city.strip()
                df.at[guest_index, "Invited"] = new_invited
                df.at[guest_index, "Amount"] = new_amount
                save_and_refresh(df)
