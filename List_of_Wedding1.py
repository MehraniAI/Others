...
if os.path.exists(DATA_FILE):
    ...
    for n in all_names:
        if n not in existing_names:
            df.loc[len(df)] = [len(df)+1, n, "", False, 0]
    # Ensure all columns exist
    for col in ["Amount", "City", "Invited"]:
...
if option == "Add New Guest":
    ...
    amount = st.number_input("Amount (PKR)", min_value=0, step=100)
    if st.button("Add Guest"):
        if name.strip() == "":
            st.warning("⚠ Name cannot be empty!")
        else:
            df.loc[len(df)] = [len(df)+1, name.strip(), city.strip(), invited, amount]
            save_and_refresh(df)
...
# ================== SEARCH GUEST ==================
    ...
    city_search = st.text_input("Filter by City/Village")
    filtered_df = df.copy()
    if name_search:
        filtered_df = filtered_df[filtered_df["Name"].str.contains(name_search, case=False, na=False)]
    if city_search:
        filtered_df = filtered_df[filtered_df["City"].str.contains(city_search, case=False, na=False)]
...
# ================== UPDATE GUEST ==================
    ...
    if not df.empty:
        ...
        if selected:
            ...
            new_name = st.text_input("New Name", df.at[idx, "Name"])
            new_city = st.text_input("New City/Village", df.at[idx, "City"])
            new_invited = st.checkbox("Invited ✅", df.at[idx, "Invited"])
            new_amount = st.number_input("Amount (PKR)", value=df.at[idx, "Amount"], min_value=0, step=100)
            if st.button("Save Update"):
                df.at[idx, "Name"] = new_name.strip()
                df.at[idx, "City"] = new_city.strip()
                df.at[idx, "Invited"] = new_invited
