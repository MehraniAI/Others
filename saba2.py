...
if st.session_state.current_view == "Add":
    ...
    st.write(f"Total books: {state['total_books']}")
    st.write(f"Books read: {state['read_books']}")
    st.write(f"Books unread: {state['unread_books']}")
    st.write(f"Percentage read: {state['percent_read']:.2f}%")